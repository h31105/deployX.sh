...
-   由于 TSP 的 SNI 分流特性，若需配置 CDN 建议使用 A 记录方式，CNAME 方式不被支持。
    ...
    CDN 配置：<域名> -- A 记录 --> VPS IP **#这里的域名与脚本部署时填写的TLS域名相同**
    ...
    3.  由于 CDN 加速**仅支持** WebSocket 模式代理，客户端需要根据脚本生成的配置信息做相应调整：
        ...
        \*TCP 模式 代理客户端 **服务器地址(ServerAddress)**为 **VPS IP 地址**或者**任意指向该 IP 地址的域名**（其他配置不变）

        \*WebSocket 模式 代理客户端 **服务器地址(ServerAddress)**为 **域名**或者**任意支持 Anycast 节点 IP**（其他配置不变）
